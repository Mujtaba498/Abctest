...
export default function CategoryPageSSR({ initialData }: CategoryPageSSRProps) {
  ...
  const [subcategory, setSubcategory] = useState<Category | null>(initialData?.subcategory || null);
  ...
  // If we have few posts, show them all in the main content
  // Only separate latest news if we have more than 4 posts
  let latestNewsPosts: Post[] = [];
  let restPosts: Post[] = [];
  
  if (sortedPosts.length <= 4) {
    // If 4 or fewer posts, show all in main content
    restPosts = sortedPosts;
    latestNewsPosts = sortedPosts.slice(0, 4); // Use same posts for sidebar
  } else {
    // If more than 4 posts, separate them
    latestNewsPosts = sortedPosts.slice(0, 4);
    restPosts = sortedPosts.filter(post => !latestNewsPosts.some(news => news._id === post._id || news.id === post.id));
  }
                  ...
                  <NavigationMenuList className="flex-nowrap gap-2 min-w-max">
                    <NavigationMenuItem>
                      <Link
                        href="/"
                        className={
                          'px-4 py-2 rounded-md text-sm font-medium transition-colors bg-white text-zinc-900 dark:bg-zinc-900 dark:text-zinc-50 hover:bg-zinc-100 dark:hover:bg-zinc-800 whitespace-nowrap inline-block'
                        }
                        prefetch={true}
                      >
                        Tout
                  ...
                  <div key={post._id || post.id} className="flex flex-col bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div className="p-4 flex-1 flex flex-col">
                      <h3 className="text-lg font-semibold mb-2 line-clamp-2">
                        <Link href={getPostUrl(post)} className="hover:underline underline-offset-2 transition-all text-zinc-900 dark:text-zinc-100">
                          {post.title}
                        </Link>
                      </h3>
                ...
                <div className="mb-2">
                  {getPostImage(leftLarge) && (
                    ...
                    className="w-full h-48 object-cover rounded mb-2"
                      ...
                      onError={() => handleImageError(leftLarge._id || leftLarge.id)}
                  />
                  )}
                  <h3 className="text-xl font-bold mb-1">
                    <Link href={getPostUrl(leftLarge)} className="hover:underline underline-offset-2 transition-all">{leftLarge.title}</Link>
                  </h3>
                </div>
              )}
              {leftSmall.map(post => (
                ...
                <div className="mb-2">
                  {getPostImage(centerXL) && (
                    ...
                    className="w-full h-64 object-cover rounded mb-2"
                      ...
                      onError={() => handleImageError(centerXL._id || centerXL.id)}
                  />
                  )}
                ...
                <div className="mb-2">
                  {getPostImage(rightLarge) && (
                    ...
                    className="w-full h-48 object-cover rounded mb-2"
                      ...
                      onError={() => handleImageError(rightLarge._id || rightLarge.id)}
                  />
                  )}
                  <h3 className="text-xl font-bold mb-1">
                    <Link href={getPostUrl(rightLarge)} className="hover:underline underline-offset-2 transition-all">{rightLarge.title}</Link>
                  </h3>
                </div>
              )}
              {rightSmall.map(post => (
